...
double lnL_MKS_PhiS_PhiMu(char model
	...
	/*****************************/
	 ...
	 for (i = 0; i < dim; ++i) {
		 xmin[i] = -cubSInf;
		 if(smax > -cubZero){ //This apply for model A. In this case, the calculation will be performed 2 times
			 xmax[i] = -cubZero;
		 }
		 else{
			 xmax[i] = smax;
...
double calc_EPn_OverPhiS_adapt(char model, double Ln, int n
		,double Mutheta, double r, double smean, double smax, double Pb, double Sb
		,double shapeBeta
		,double cubTol, int cubMaxEval, double cubZero, double cubSInf){
	...
	f_integrand = (int *) malloc(sizeof(int));

	// Optional: to extend the calc for ESn and EDn, just change the f_integrand bellow to 1 or 2 respectively
	f_integrand[0] = 0;
	...
	/*****************************/
     ...
     valSum = (double *) malloc(sizeof(double));
     ...
     cubature(1, integrands_phi, &fparams,
	      dim, xmin, xmax,
	      maxEval, 0, tol, ERROR_INDIVIDUAL, val, err);
     ...
     // model_A : Integral over Phi
   	  ...
   	  cubature(1, integrands_exp, &fparams,
   		  dim, xmin, xmax,
   		  maxEval, 0, tol, ERROR_INDIVIDUAL, val, err);
...
double calc_ESn_OverPhiS_adapt(char model, double Ln, int n
		,double Mutheta, double smean, double smax, double Pb, double Sb
		,double shapeBeta
		,double cubTol, int cubMaxEval, double cubZero, double cubSInf){
	...
	/*****************************/
     ...
     // model_A : Integral over Phi
         ...
         //model_B : Constant
         // Sum of val
         valSum[0] += Pb * ESn_ana(Ln,Mutheta,n,Sb);
...
double calc_EDn_OverPhiS_adapt(char model, double Ln, int n
	...
	/*****************************/
     ...
     // model_A : Integral over Phi
         ...
         //model_B : Constant
         // Sum of val
         valSum[0] += Pb * EDn_ana(Ln,lambda,Mutheta, r, n,Sb);
